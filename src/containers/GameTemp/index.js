...
class GameTemp extends Component {
  ...
  componentDidMount() {
    ...
    debug('CtGameTemp')(`Component mounted, received props:`);
    debug('CtGameTemp')(this.props);
  }
  ...
  setTexture = (textureName, key, texture) => {
    debug('CtGameTemp')(`Loaded texture, setting: ${textureName}, ${key}`);
  ...
  keyPressed = (evt) => {
    debug('CtGameTemp')(`Pressed a key Sir: ${evt.keyCode}`);
    if (evt.keyCode === 27) {
      debug('CtGameTemp')(`Pressed game pause, trying to pause the game:`);
...
export default connect(mapStateToProps, mapDispatchToProps)(GameTemp);
